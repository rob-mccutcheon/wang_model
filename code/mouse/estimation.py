...
data_dir = f'/users/k1201869/wang_model/data/mouse/'

#Set where to save parameters and correlation
results_dir = f'/users/k1201869/wang_model/results/mouse/allenSC'

# Number of fitting iterations
EstimationMaxStep = 150

# Load FC and SC
fc_file = f'{data_dir}/{subject}/cm.txt'
# SC = np.loadtxt(open(f"/users/k1201869/wang_model/data/SC_dk.csv", "rb"), delimiter=",")
SC = np.loadtxt(open(f"/users/k1201869/wang_model/data/mouse/Allen_count500_tvb/weights.txt", "rb"), delimiter=" ")
#delete if not indvi
...
init_version = 0
try:
    init_version = int(max(os.listdir(f'/users/k1201869/wang_model/temp/mouse/allenSC/{subject}')))
    saved_variables_filename = os.listdir(f'/users/k1201869/wang_model/temp/mouse/allenSC/{subject}/{init_version}')[0]
    saved_variables = pickle.load(open(f'/users/k1201869/wang_model/temp/mouse/allenSC/{subject}/{init_version}/{saved_variables_filename}', 'rb'))
...
for version in range(init_version, version_max):
    ...
    #    JK = JFK[:, p:]
        ...
        temp_directory = f'/users/k1201869/wang_model/temp/mouse/allenSC/{subject}/{version}'
