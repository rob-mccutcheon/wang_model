...
#data_dir = '/p/project/tvbdmwm/44_Rob_SCZ_EIbalance_Wang/export'
...
for version in range(5):
    ...
    while step <= EstimationMaxStep:
        ...
        cluster = SLURMCluster(cores=1, memory=mem, 
                ...
                log_directory=f'./dask_logs2_{subject}')
    ...
    #    JF = JFK[:, :p] # {nT x p}
    #    JK = JFK[:, p:]
        # JF = Parallel(n_jobs=-1)(delayed(wf.CBIG_MFMem_rfMRI_diff_P1)(funcA,A,h_output,i) for i in range(p))
        # JK = Parallel(n_jobs=-1)(delayed(wf.CBIG_MFMem_rfMRI_diff_PC1)(funcA,A,i) for i in range(p))
        ...
        JFK[:,:]=np.array(res1).T
        client.shutdown()
        shutil.rmtree(f'/users/k1201869/wang_model/dask_logs2_{subject}')
    ...
    outfile = f'{results_dir}/output_bifur2_{subject}_{version}.txt'
