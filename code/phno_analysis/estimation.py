...
subject_list = np.squeeze(pd.read_table("/users/k1201869/wang_model/code/phno_analysis/subjects.list", header=None).values)
subject = subject_list[subject_idx][:3]
session = subject_list[subject_idx][3]
print(f'subject {subject} session {session}')

# Set paths for where structural and functional connectivity matrices are stored
data_dir = f'/users/k1201869/wang_model/data/phno/'

#Set where to save parameters and correlation
results_dir = f'/users/k1201869/wang_model/results/phno/indivSC'

# Number of fitting iterations
EstimationMaxStep = 150

# Load FC and SC (loaded lacking the corpus callosum)
fc_file = f'{data_dir}/dk_fc/sub-{subject}_ses-{session}_FC_dk.csv'
# SC = np.loadtxt(open(f"/users/k1201869/wang_model/data/SC_dk.csv", "rb"), delimiter=",")
SC = np.loadtxt(f'{data_dir}/dk_sc/{subject}_SC_dk.csv')
#delete if not indvi
...
idx = np.hstack((np.arange(1,36), np.arange(37,70)))
FC = FC[idx][:,idx]
...
init_version = 0
try:
    init_version = int(max(os.listdir(f'/users/k1201869/wang_model/temp/phno/indivSC/{subject}')))
    saved_variables_filename = os.listdir(f'/users/k1201869/wang_model/temp/phno/indivSC/{subject}/{init_version}')[0]
    saved_variables = pickle.load(open(f'/users/k1201869/wang_model/temp/phno/indivSC/{subject}/{init_version}/{saved_variables_filename}', 'rb'))
...
for version in range(init_version, version_max):
    ...
    while step <= (EstimationMaxStep-1):
        ...
        log_directory=f'/users/k1201869/wang_model/dask_logs/hcpep/dask_logs_{subject}')
    ...
    #    JK = JFK[:, p:]
        ...
        JFK[:,:]=np.array(res1).T
        client.shutdown()
        shutil.rmtree(f'/users/k1201869/wang_model/dask_logs/hcpep/dask_logs_{subject}', ignore_errors=True)
        JF = JFK[:, :p] # {nT x p}
        JK = JFK[:, p:]
        ...
        if breakloop ==1: #break if there was a linalg error
            break
        ...
        temp_directory = f'/users/k1201869/wang_model/temp/phno/indivSC/{subject}/{version}'
