...
data_dir = f'/users/k1201869/wang_model/data/hcp_scz/1002_01_MR'

#Set where to save parameters and correlation
results_dir = f'/users/k1201869/wang_model/results/glasser'
...
for version in range(init_version, 5):
    ...
    while step <= (EstimationMaxStep-1):
        ...
        log_directory=f'/users/k1201869/wang_model/dask_logs/hcp_testretest/test/dask_logs_{subject}')
    ...
    #    JK = JFK[:, p:]
        ...
        JFK[:,:]=np.array(res1).T
        client.shutdown()
        shutil.rmtree(f'/users/k1201869/wang_model/dask_logs/hcp_testretest/test/dask_logs_{subject}', ignore_errors=True)
        JF = JFK[:, :p] # {nT x p}
        JK = JFK[:, p:]
