...
results_dir = f'/users/k1201869/wang_model/results/hcp_testretest/single_params_long/{dataset}'
...
p = 2 + 2 # number of estimated parametera

init_version = 0
try:
    init_version = int(max(os.listdir(f'/users/k1201869/wang_model/temp/hcp_testretest/single_params_long/{dataset}/{subject}')))
    saved_variables_filename = os.listdir(f'/users/k1201869/wang_model/temp/hcp_testretest/single_params_long/{dataset}/{subject}/{init_version}')[0]
    saved_variables = pickle.load(open(f'/users/k1201869/wang_model/temp/hcp_testretest/single_params_long/{dataset}/{subject}/{init_version}/{saved_variables_filename}', 'rb'))
    print('loaded saved variables')
except:
    print('no variables loaded')


for version in range(init_version, 6):
    ...
    A_start = -1/4+(1/4+1/4)*np.random.rand(2+2,1)
    ...
    while step <= (EstimationMaxStep-1):
        ...
        def funcP(Para_E, Prior_E=Prior_E,SC=SC,y=y,FC_mask=FC_mask,Nstate=Nstate,Tepochlong=14.4,TBOLD=0.72,ifRepara=0):
            FC_simR, CC_check = wf.CBIG_MFMem_rfMRI_nsolver_eul_sto(Para_E,Prior_E,SC,y,FC_mask,Nstate,Tepochlong,TBOLD,ifRepara, single_param=True)
        ...
        def funcA(A, Prior_E=Prior_E,SC=SC,y=y,FC_mask=FC_mask,Nstate=Nstate,Tepochlong=14.4,TBOLD=0.72,ifRepara=1):
            FC_simR, CC_check = wf.CBIG_MFMem_rfMRI_nsolver_eul_sto(A,Prior_E,SC,y,FC_mask,Nstate,Tepochlong,TBOLD,ifRepara, single_param=True)
            return FC_simR, CC_check
    ...
    #    JK = JFK[:, p:]
        ...
        for i in range(2*p):
            if i<p: # it is alright to get warning about losing the imaginary part for P1 but not alright for PC1
                res = wf.CBIG_MFMem_rfMRI_diff_P1(funcA,A,h_output,i)
                res1.append(res)
            else:
                res = wf.CBIG_MFMem_rfMRI_diff_PC1(funcA,A,i-p)
                res1.append(res)
        ...
        temp_directory = f'/users/k1201869/wang_model/temp/hcp_testretest/single_params_long/{dataset}/{subject}/{version}'
