...
for version in range(10):
    ...
    while step <= (EstimationMaxStep-1):
        ...
        cluster = SLURMCluster(cores=1, memory=mem, 
                        ...
                        log_directory=f'./dask_logs_{subject}')
        if atlas=='dk':        
        ...
        client.shutdown()
        shutil.rmtree(f'/users/k1201869/wang_model/dask_logs_{subject}')
        ...
        # import pickle
        # JFK2=pickle.load(open('./jfk', 'rb'))
        # JFK = pickle.load(open('./jfk_glasser', 'rb'))
        #pickle.dump(JFK, (open('./jfk_glasser', 'wb')))
        ...
        #-------------------commence M-step loop %----------------------------------
            for mi in range(16):
                ...
                P = np.zeros([nT,p])
                #step1: inv(Ce) * J  
                print(f'start mi{mi} range(p) 2 loop')
                print(datetime.now().strftime("%H:%M:%S"))
                for i in range(p):
                        # h_output2=[da.from_array(h_output, chunks=h_output.shape[0]//20)]
                    P[:,i] = (J[:,i]*np.squeeze(inv_DiagCe))
                print(f'finish mi{mi} range(p) 2 loop')
                print(datetime.now().strftime("%H:%M:%S"))
                #step2: (inv(Ce) * J) * inv_JinvCeJ * J'   
                P = np.matmul(np.matmul(P,inv_JinvCeJ),J.T)
                #step3:  -(inv(Ce) * J * inv_JinvCeJ * J') * inv(Ce) 
                
                print(f'start mi{mi} range(NT) loop')
                print(datetime.now().strftime("%H:%M:%S"))
                # for i in range(nT):
                ...
                #                         d  lembda(i)
                # see [2,3]
                print(f'start mi{mi} range(n) loop')
                print(datetime.now().strftime("%H:%M:%S"))      
                import sys
                ...
                for i in range(n):
                    ...
                    #step3:
                    g[i] = g[i] + g_rest
                print(f'finish mi{mi} range(n) loop')
                print(datetime.now().strftime("%H:%M:%S"))   
        ...
        print(f'end pa{datetime.now().strftime("%H:%M:%S")}')
    ...
    np.savetxt(f'{results_dir}/final_parameters_{subject}_{atlas}_{version}.txt', final_results)
